...
/**
 * @ClassName HeartBeatTypeHandler
 * @Description TODO
 * @Author wangjinfei
 * @Date 2025/6/16 20:19
 */
@Slf4j
@Component
public class HeartBeatTypeHandler implements MessageTypeHandler {
    private final Map<String, SubTypeHandler> subHandlerMap = new HashMap<>();

    // 构造器注入所有的heart_beat子类型处理器
    public HeartBeatTypeHandler(List<SubTypeHandler> allSubHandlers) {
        for (SubTypeHandler subHandler : allSubHandlers) {
            if(MessageType.HEART_BEAT.getType().equals(subHandler.getMessageType())){
